...
class FormOpenEndedQuestion extends FormQuestionWidget
{
    ...
    public function __set($strKey, $varValue)
    {
        switch ($strKey) {
            ...
            case 'value':
                ...
                if (strcmp($this->questiontype, 'oe_multiline') != 0) {
    ...
    protected function setData_oe_singleline($varValue)
    {
        if (strlen($varValue['openended_width'])) {
            $this->arrAttributes["size"] = specialchars($varValue['openended_width']);
        }
        if (strlen($varValue['openended_maxlen'])) {
            $this->arrAttributes["maxlength"] = specialchars($varValue['openended_maxlen']);
        }
        if (strlen($varValue['openended_textinside'])) {
            $this->arrAttributes["value"] = specialchars($varValue['openended_textinside']);
        }
        if (strlen($this->varValue)) {
            $this->arrAttributes["value"] = specialchars($this->varValue);
        }
    }
    ...
    protected function setData_oe_multiline($varValue)
    {
        if (strlen($varValue['openended_rows'])) {
            $this->arrAttributes["rows"] = specialchars($varValue['openended_rows']);
        }
        if (strlen($varValue['openended_cols'])) {
            $this->arrAttributes["cols"] = specialchars($varValue['openended_cols']);
        }
        if (!strlen($this->varValue)) {
            if (strlen($varValue['openended_textinside'])) {
